...
public class ProtocolAttack8Controller : MonoBehaviour {
    ...
    public void Intercept() {
        intercepted = true;
        if (frameworkControl.latestMessage.message.Equals("{ M }<sub><size=130%>Key(N<size=90%>A</size>,N<size=90%>B</size>)</size></sub>")) {
            frameworkControl.Success("Well done!");
            return;
        }
    }
    ...
    private void AliceReply(CapturedMessage msg) {
        if (msg.from.Equals("Bob") || msg.alias.Equals("Bob")) {
            ...
            if (lastStepAC == 2 && msg.message.Equals("E<sub>A</sub>( " + GetNonce("Bob") + " )")) {
                frameworkControl.NewStep();
                lastStepAC = 3;
                CapturedMessage newMessage = new CapturedMessage("AliceCarolMessage" + lastStepAC, "E<sub>C</sub>( " + GetNonce("Bob") + " )", "Alice", "Carol", "Carol");
    ...
    private void BobReply(CapturedMessage msg) {
        if (msg.from.Equals("Alice") || msg.alias.Equals("Alice")) {
            if (lastStepAB == 1 && msg.message.Equals("E<sub>B</sub>( " + GetNonce("Alice") + " , A )")) {
                frameworkControl.NewStep();
                lastStepAB = 2;
                CapturedMessage newMessage = new CapturedMessage("AliceBobMessage" + lastStepAB, "E<sub>A</sub>( " + GetNonce("Bob") + " )", "Bob", "Alice", "");
                frameworkControl.latestMessage = newMessage;
            ...
            if (lastStepAB == 3 && msg.message.Equals("E<sub>B</sub>( " + GetNonce("Bob") + " )")) {
                frameworkControl.NewStep();
                lastStepAB = 4;
                CapturedMessage newMessage = new CapturedMessage("AliceBobMessage" + lastStepAB, "{ M }<sub><size=130%>Key(N<size=90%>A</size>,N<size=90%>B</size>)</size></sub>", "Bob", "Alice", "");
                frameworkControl.latestMessage = newMessage;
                lastAliceBobMsg = newMessage;
                frameworkControl.lastStepControl.SetMessageArrow("Bob", "Alice", frameworkControl.latestMessage.GetMessage(), "");
                return;
